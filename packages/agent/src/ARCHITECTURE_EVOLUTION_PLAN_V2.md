...
- 通过 `sendMessage` / `sendUserMessage` 主动触发或排队消息
...
type AgentEvent =
    | ...
    | { type: "state_change"; state: AgentState; changed: Array<keyof AgentState | "telemetry"> };
```
...
- `sendMessage(..., { triggerTurn: true })` 发送隐式 custom message
- 或 `sendUserMessage()` 发送内部 user message
...
3. 使用 `CustomEntry` 持久化 autonomy state
4. 使用 `CustomMessageEntry` 注入 memory 和 A2A 消息
5. 基于 `isIdle()`、`hasPendingMessages()`、`getContextUsage()` 做安全 heartbeat
6. 通过 `session_before_compact` 验证记忆与 compaction 的协同
