...
export class ScriptProbe implements StateProbe<Record<string, unknown>> {
	readonly key: string;
	...
	async collect(ctx: ExtensionContext): Promise<ProbeResult<Record<string, unknown>>> {
		const sampledAt = Date.now();
		const execution = await this.executeScript(ctx);
	...
	private async executeScript(ctx: ExtensionContext): Promise<ScriptExecutionResult> {
		return await new Promise((resolve) => {
			const child = spawn(this.definition.command, this.definition.args ?? [], {
				cwd: this.definition.cwd ?? ctx.cwd,
				env: {
					...process.env,
					...this.definition.env,
				},
